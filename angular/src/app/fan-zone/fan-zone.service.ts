...
export class FanZoneService {
    ...
    getAlbuns(){
        this.http.get<{albuns: AlbumData[]}>(environment.webRestUrl + 'fan-zone/get-albuns/' )
            .pipe(
                catchError(this.handleError)
            ).subscribe( albunsData => {
                console.log(albunsData);
                this.albuns.next(albunsData.albuns)
            })
    }

    getAccessToAlbum(albumId : string){
        this.http.post<string>(environment.webRestUrl + 'fan-zone/post-access-to-album' , {albumId: albumId})
            .subscribe(response => {
                this.getAlbuns()
            })
    }
